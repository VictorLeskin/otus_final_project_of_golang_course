...
По условиям задания разрешено использование сторонних библиотек. Для работы с IPv4 и CIDR стандартная библиотека golang предлагает пакет net. 
Предполагается использовать его для проверки правильности данного IP адреса, подсети и проверки принадлежность IP адреса сети.
Основные функции из пакета net для работы c blacklist и whitelist.
1. Парсинг IP-адреса: net.ParseIP()
2. Парсинг подсети (CIDR): net.ParseCIDR()
3. Проверка принадлежности сети: IPNet.Contains()

Разрешение конфликта blacklist и whitelist.
Предполагается , что whitelist — это то, что мы разрешаем. Если появляется blacklist, значит это запрещенные адреса внутри разрешенной зоны.
Если адрес попадает в оба списка предполагается реализовать в этом случае Приоритет Blacklist как самый безопасный. Потому что он защищает от ошибок конфигурации. 
